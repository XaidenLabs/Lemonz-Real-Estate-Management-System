...
const StatBox = ({ label, value }) => (
  <View className="items-center">
    <Text className="text-xl text-white font-rbold">{value}</Text>
    <Text className="text-xs text-gray-400 font-rregular">{label}</Text>
  </View>
);

const Profile = () => {
  const { getUser, user, logout } = useAuth();
  // Assuming useProperty has numberOfProperties, if not we can remove this or use a placeholder
  const { numberOfProperties } = useProperty();
      ...
      >
          ...
          <SafeAreaView className="items-center justify-center flex-1 px-4">
            <View className="absolute top-4 right-4">
              <TouchableOpacity
                ...
                onPress={() => router.push("/agent/profile/edit")}
              >
            ...
            <Text className="text-2xl text-white font-rbold">
              {user.companyName || user.name || "Agent"}
            </Text>
            ...
            <View
              ...
              className="flex-row items-center justify-between w-full p-4 mt-6 border rounded-2xl"
            >
              <StatBox label="Properties" value={numberOfProperties || 0} />
              <View className="h-8 border-l border-gray-700" />
              <StatBox label="Rating" value="4.8" />
              <View className="h-8 border-l border-gray-700" />
              <StatBox label="Reviews" value="124" />
            </View>
          </SafeAreaView>
        </View>
        ...
        <View className="px-4 mt-4">
          ...
          <ProfileOption
            ...
            label="Verify ID"
            onPress={() => router.push("/agent/verify-id")}
          />
          <ProfileOption
            icon="card-outline"
            label="Subscription"
